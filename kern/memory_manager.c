...
{
	int temp,temp2;
	temp=ROUNDUP(size,1024*4);
	temp2=temp/(1024*4);
	// allocate , map pages of given size
	for(int i=0;i<temp2;++i)
	{
		int ret = pf_add_empty_env_page(e,virtual_address,0);
		virtual_address+=(1024*4);
	}
...
void table_entry(struct Env* e,struct WorkingSetElement *el,uint32 *p_page_table){
    //loop on all entry table
    int cnt_entry_empty = 0;
    for(int i =0 ; i< 1024; i++)
    {
    	if(p_page_table[i]==0) { cnt_entry_empty++; }
    	else {	break; }
    }
    //if 1024 entry == 0
    if(cnt_entry_empty==1024)
    {
    	struct Frame_Info *p_frame_info  = get_frame_info(e->env_page_directory,(void*) p_page_table, &p_page_table);
		free_frame(p_frame_info);
		//remove page table from dir table
		pd_clear_page_dir_entry(e, el->virtual_address);
    }
}
...
int round = ROUNDUP(size, PAGE_SIZE);
uint32 number_of_frames=round/PAGE_SIZE;
uint32 srart_add=virtual_address;
uint32 end_add = virtual_address + round;
int counter1 = 1;
//cprintf("%d\n",size);
//remove from page file
	while(counter1 <= number_of_frames)
	{
		pf_remove_env_page(e,srart_add);
		srart_add+=PAGE_SIZE;
		counter1++;
	}

	//2. Free ONLY pages that are resident in the working set from the memory
	srart_add = virtual_address;
	struct WorkingSetElement *el;
	struct WorkingSetElement *el2;
	//loop in active list
	LIST_FOREACH(el,&(e->ActiveList))
	{
		//if virtual of active between start and end
		if(el->virtual_address>=srart_add && el->virtual_address < end_add)
		{
			uint32* p_page_table;
			get_page_table(e->env_page_directory,(void*)el->virtual_address, &p_page_table);
			unmap_frame((e->env_page_directory),(void*)el->virtual_address);
			el->empty = 1;
			//remove el from active
			LIST_REMOVE(&(e->ActiveList),el);
			//insert in head
			LIST_INSERT_HEAD(&(e->PageWorkingSetList),el);
			//if there el in second remove and add to active
			if(LIST_SIZE(&(e->SecondList))!=0)
			{
				//move el from second to active
				struct WorkingSetElement *elsec=LIST_FIRST(&(e->SecondList));
				LIST_REMOVE(&(e->SecondList),elsec);
				LIST_INSERT_TAIL(&(e->ActiveList),elsec);
				pt_set_page_permissions(e,elsec->virtual_address,PERM_PRESENT,0);
			}
			//remove entry from page table
		    pt_clear_page_table_entry(e, el->virtual_address);
		    //3. Removes ONLY the empty page tables (i.e. not used) (no pages are mapped in the table)
		    table_entry(e,el,p_page_table);

		}

	}
//if address in second list
	LIST_FOREACH(el2,&(e->SecondList))
	{
		//if virtual of active between start and end
		if(el2->virtual_address>=srart_add && el2->virtual_address < end_add){
			uint32* p_page_table;
			get_page_table(e->env_page_directory,(void*)el2->virtual_address, &p_page_table);
			//unmap virtual
			unmap_frame((e->env_page_directory),(void*)el2->virtual_address);
			el2->empty = 1;
			//remove from second
			LIST_REMOVE(&(e->SecondList),el2);
			//insert el to workingset
			LIST_INSERT_HEAD(&(e->PageWorkingSetList),el2);
			//pt_set_page_permissions(e,el2->virtual_address,PERM_PRESENT|PERM_WRITEABLE,0);
			//remove entry from page table
			pt_clear_page_table_entry(e, el2->virtual_address);
			//3. Removes ONLY the empty page tables (i.e. not used) (no pages are mapped in the table)
			table_entry(e,el2,p_page_table);
		}
	}
